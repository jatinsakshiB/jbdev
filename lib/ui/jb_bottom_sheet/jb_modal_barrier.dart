...
/// See also:
///
///  * [JBModalBarrier], which utilizes this widget to adjust the barrier focus
...
///  * [JBAnimatedModalBarrier], which is similar but takes an animated [color]
///    instead of a single color value.
class JBModalBarrier extends StatelessWidget {
  /// Creates a widget that blocks user interaction.
  const JBModalBarrier({
    super.key,
    this.color,
    this.blur,
  ...
  ///    [JBModalBarrier] built by [ModalRoute] pages.
  final Color? color;
  ...
  ///    the [JBModalBarrier] built by [ModalRoute] pages.
    ...
    Widget barrier = Semantics(
      ...
      child: MouseRegion(
        ...
        child: ConstrainedBox(
          ...
          child: blur == null ? color == null ? null : ColoredBox(color: color!) : BackdropFilter(
            filter: ImageFilter.blur(sigmaX: blur!, sigmaY: blur!),
            child: ColoredBox(color: color ?? Colors.black54),
          ),
        ),
      ),
    );
...
/// usually darkened by the modal barrier.
///
/// This widget is similar to [JBModalBarrier] except that it takes an animated
/// [color] instead of a single color.
///
/// See also:
///
///  * [ModalRoute], which uses this widget.
class JBAnimatedModalBarrier extends AnimatedWidget {
  /// Creates a widget that blocks user interaction.
  JBAnimatedModalBarrier({
    super.key,
    required this.color,
    this.blur,
    this.dismissible = true,
    ...
    this.clipDetailsNotifier,
    this.semanticsOnTapHint,
  }) : super(listenable: Listenable.merge([color, blur]));
  ...
  ///
  ///  * [ModalRoute.barrierColor], which controls this property for the
  ///    [JBAnimatedModalBarrier] built by [ModalRoute] pages.
